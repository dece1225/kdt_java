...
public class TypeCasting02 {//클래스 중괄호 영역 시작
	...
	public static void main(String[] args) {// main 메소드 중괄호 영역 시작
		...
		//1) 실수 -> 정수
		double num1 = 3.141592; //double 변수에 3.141592값을 대입
		int result1 = (int)num1; // int형 변수에 int형 변수로 강제적으로 형변환후 대입
		System.out.println(num1); // 3.141592 값을 출력후 줄바꿈한다
		System.out.println(result1);// 3값을 출력후 줄바꿈한다.
		System.out.println("실수형 num1의 값 : "+num1
				+", 정수형으로 변환된 값 : " + result1);// 실수형 num1의 값 : 3.141592, 정수형으로 변환된 값 : 3 출력후 줄바꿈
		
		//2) 큰 범위 정수 -> 작은 범위 정수
		long num2 = 10L;	//8byte / long 변수선언후 10L를 대입
		int result2 = (int)num2;	//4byte 변수 선언후 int로 형변환후 대입
		...
		System.out.println("long타입 num3 값 : "+num3
				+ ", 왜곡된 값 : "+ result3);
		// 음수가 나타나는 이유(overflow의 원리)
		// int의 범위는 32bit 자료형 
		// 값의 범위 -2,147,483,648 ~ 2,147,483,647
		// long에서 int로 변환 => 2,147,483,648은 int의 범위를 초과하므로 상위비트(초과된 비트)가 잘리게된다
