...
public class UserDAO {
	...
	public int login(String userId, String userPw) {
		...
		try {
			...
			System.out.println("login() SQL 오류발생!!!");
			e.printStackTrace();
		} finally {
			try {
				if(resultSet != null) {
	...
	//아이디 찾기
	//반환타입을 List인 이유 : 이름/생일이 같은 사람이 여러명일수 있으므로 List로 반환
	...
	public void update(UserDTO userDTO) {
		String query = "UPDATE TBL_USER SET USER_ID = ? , USER_NAME = ? "
				+ ", USER_AGE = ? , USER_GENDER = ? , USER_BIRTH = ? , USER_NUMBER = ?"
				+ "WHERE USER_NUMBER = ?";
